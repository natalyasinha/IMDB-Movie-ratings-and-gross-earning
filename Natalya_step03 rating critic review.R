...
top10_movies$no.of.pages.of.review <- c(31,48,28,34,38,42,33,32,37,17)
bottom10_movies$no.of.pages.of.review <- c(2,8,3,2,5,4,1,5,10,8)
...
for(i in 1:10){
  ...
  for(j in 1:No.of.pages){
    if (j==1){
      ...
      URL <- paste0("http://www.imdb.com/title/tt", ID, "/reviews?filter=prolific;filter=prolific;start=", (j-1)*10)
  ...
  mypath <- file.path(My.Location,"/Top10",paste("frequency_plot_",i, ".jpg", sep = ""))
  ...
  mypath <- file.path(My.Location,"/Top10",paste("wordcloud_",i, ".jpg", sep = ""))
...
mypath <- file.path(My.Location,"/Top10",paste("Sentiment Analysis Top 10.jpg", sep = ""))
...
mypath <- file.path(My.Location,"/Top10",paste("Net Positive Sentiment Analysis Top 10.jpg", sep = ""))
jpeg(file=mypath)
p <- movie.sentiment %>%
  ...
  ylab('Net Positive Sentiment')+xlab('Movies - Top 10')
print(p)
dev.off()
...
mypath <- file.path(My.Location,"/Top10",paste("Sentiment Analysis Top 10 v2.jpg", sep = ""))
jpeg(file=mypath)
p <- movie.sentiment %>%
...
for(i in 1:10){
  ...
  mypath <- file.path(My.Location,"/Bottom10",paste("frequency_plot_",i, ".jpg", sep = ""))
  ...
  mypath <- file.path(My.Location,"/Bottom10",paste("wordcloud_",i,  ".jpg", sep = ""))
...
mypath <- file.path(My.Location,"/Bottom10",paste("Sentiment Analysis Bottom 10.jpg", sep = ""))
...
mypath <- file.path(My.Location,"/Bottom10",paste("Net Positive Sentiment Analysis Bottom 10.jpg", sep = ""))
jpeg(file=mypath)
p <- movie.sentiment %>%
  dplyr::select(sentiment,relative.sentiment,movie.name) %>%
  spread(sentiment,relative.sentiment) %>%
...
mypath <- file.path(My.Location,"/Bottom10",paste("Sentiment Analysis Bottom 10 v2.jpg", sep = ""))
jpeg(file=mypath)
p <- movie.sentiment %>%
  filter(!sentiment %in% c("disgust","fear")) %>%
  ggplot(aes(x=sentiment,y=relative.sentiment,fill=sentiment)) + geom_bar(stat='identity') + 
